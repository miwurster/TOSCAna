...
/**
 * Extend from this class in order to inherit important configurations Sets up Spring Test EntityContext regarding to
 * the Test Configuration After every test method, refreshes the context. If disk access is needed, use {@link #tmpdir}
 */
...
public abstract class BaseSpringTest extends BaseTest {
    ...
    @Before
    public final void startupPrepareDisk() {
    ...
    @After
    public final void shutdownCleanUpDisk() {
        FileSystemUtils.deleteRecursively(tmpdir);
        logger.info("Clean up disk");
    }
}
